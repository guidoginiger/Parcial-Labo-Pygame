...
import botones
import puntuaciones
from pedir_texto import ingresar_nombre
from pantalla_final import pantalla_cierre
...
class Snake:
	...
	def crear_tiempo(self):
		...
		contador = fuente.render(segundos, True, (colores.NEGRO))
...
class Main:
	...
	def pantalla_reset(self):
		#PEDIR NOMBRE AL JUGADOR / MOSTRAR PUNTUACIONES
		#CREAR UNA NUEVA PANTALLA
		nombre_jugador = ingresar_nombre(pantalla)
		scores = self.crear_score()
		scores= int(scores)
		pantalla_cierre(pantalla, nombre_jugador, scores)		
...
fondo_intro = pygame.image.load("Imagenes/fondo_intro.png").convert_alpha()
fondo_intro = pygame.transform.scale(fondo_intro, (700, 900))
...
while running:
	...
	while intro:
		...
		for event in pygame.event.get():
			...
			titulo = font.render("BIENVENIDO/A", True, (colores.CELESTE))
			inst_jugar = font.render("Presione ENTER para JUGAR...", True, (colores.CELESTE))
			inst_salir = font.render("Presione E para SALIR...", True, (colores.CELESTE))
	...
	while esta_jugando:
		...
		if puntaje < 3:
			...
			pantalla.blit(imagen_snake, imagen_fondo_pos)
			final = fuente.render("FINAL", False, (colores.NEGRO), "Red")
			pantalla.blit(final, (200, 0))
			tiempo.tick(17)
			juego_principal.update()
